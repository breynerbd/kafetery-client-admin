...
export const RestaurantModal = ({ isOpen, onClose, restaurant }) => {
    ...
    useEffect(() => {
        if (restaurant) {
            ...
            setFormData({ name: "", description: "", address: "", phone: "", email: "", owner: "" });
        }
    }, [restaurant, isOpen]);
    ...
    return (
        <div className="fixed inset-0 bg-[#4A3728]/60 backdrop-blur-sm flex justify-center items-center z-[100] p-4 sm:p-6">
            {/* Contenedor Principal: max-h-[90vh] para que nunca exceda la pantalla y overflow-hidden */}
            <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300 border border-[#EADDCA]/50">
                ...
                {/* Cuerpo con Scroll: overflow-y-auto permite bajar si el contenido es mucho */}
                ...
                <div className="p-6 sm:p-8 border-t border-[#EADDCA]/30 bg-white shrink-0">
                    <div className="flex flex-col gap-3">
                        <button className="w-full py-3.5 rounded-xl bg-[#4A3728] text-white hover:bg-[#6F4E37] transition font-bold shadow-lg text-sm uppercase tracking-widest">
                            {restaurant ? "Actualizar Sucursal" : "Registrar en Sistema"}
                        </button>
