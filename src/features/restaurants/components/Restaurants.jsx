...
import { Store, Plus, MapPin, Phone, Navigation, Edit3, Trash2 } from "lucide-react";
...
export const Restaurants = () => {
    ...
    const openMap = (restaurant) => {
        ...
        if (!lat || !lng) {
            showError("Este restaurante aún no tiene ubicación.");
            return;
        }
                                ...
                                <tr key={res._id || res.id} className="hover:bg-[#FDF8F3]/50 transition-colors group">
                                    ...
                                    <td className="px-8 py-5">
                                        <div className="flex flex-col text-xs gap-1">
                                            <button
                                                onClick={() => openMap(res)}
                                                className="flex items-center gap-2 text-[#8B4513] hover:text-[#4A3728] font-bold"
                                            >
                                                <Navigation size={14} />
                                                Ver ubicación
                                            </button>
                    ...
                    {restaurants.length > 0 ? restaurants.map((res) => (
                        <div key={res._id || res.id} className="p-6 flex flex-col gap-4">
                            ...
                            <div className="space-y-2 border-l-2 border-[#EADDCA] pl-4 py-1">
                                <button
                                    onClick={() => openMap(res)}
                                    className="flex items-center gap-2 text-sm text-[#8B4513] font-bold hover:text-[#4A3728]"
                                >
                                    <Navigation size={15} />
                                    Ver ubicación
                                </button>
